IF EXISTS (SELECT name FROM sysobjects WHERE name = N'P_AllData_ETL' AND type = 'P')
   DROP PROCEDURE P_AllData_ETL
GO

Create PROCEDURE P_AllData_ETL
...
--Select @S=GetDate()  
--  Exec P_infoRYDJB_ETL @BigStartDate, @EndDate -- HIS入院登记表
--Select @E=GetDate()
...
begin try      
  Exec P_VsTjZy_ETL @BigStartDate, @EndDate 
end Try              
begin catch              
 insert into error_Log(ZxDate,error_step,error_text)               
